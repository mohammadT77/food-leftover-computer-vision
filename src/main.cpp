...
int main() {
    ...
    Mat image = imread("../data/bow_dictionary/1_tray1.jpg");
    vector<DistanceIndexPair> result;
    result = process_image(image, breadsalad_words, breadsalad_bow);
    for (const auto& e:result) {
        cout << e << ',';
    } cout << endl;
    result = process_image(image, trainDists, breadsalad_words, breadsalad_bow);
